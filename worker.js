...
async function getLiveMovies() {
  ...
  if (!multichainMap) {
    return { ok: true, source: '영화진흥위원회 영화관입장권통합전산망(KOBIS) 공식 데이터', basedAt: getKoreaDate(), live: true, cinemaCheckedAt: null, movies: [] };
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/movies') {
      try {
        return json(await getLiveMovies(), 200, { 'cache-control': 'no-store' });
      } catch (error) {
        console.error('MOVOKA KOBIS live data error:', error);
        return json({ ok: false, code: 'KOBIS_LIVE_DATA_ERROR', message: 'KOBIS 공식 영화 데이터를 불러오지 못했습니다.' }, 502, { 'cache-control': 'no-store' });
      }
    }
    ...
    if (url.pathname === '/internal/refresh-cinema-links' && request.method === 'POST') {
      return json({ ok: false, disabled: true, message: '영화관 체인 데이터는 KOBIS 공식 멀티체인 통계에서만 조회합니다.' }, 501);
    }

    if (url.pathname === '/internal/refresh-movies' && request.method === 'POST') {
      return json({ ok: false, disabled: true, message: 'KOBIS 결과를 저장하지 않는 실시간 조회 구조입니다. /api/movies를 사용하세요.' }, 410);
    }
