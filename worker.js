...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/performances') {
      ...
      // 첫 화면만 캐시를 사용하고, 2페이지 이상은 반드시 KOPIS에서 새로 조회합니다.
    ...
    if(url.pathname==='/'||url.pathname==='/index.html'){
      ...
      // 기존 load()가 먼저 실행해도 페이지네이션 로더가 최종적으로 이벤트를 다시 연결합니다.
...
function loadPage(page){page=Math.max(1,Number(page)||1);var grid=document.querySelector('#grid'),count=document.querySelector('#count'),el=document.querySelector('#pagination');if(!grid)return;grid.innerHTML='<div class="empty">공연 정보를 불러오는 중입니다.</div>';var f=getFilters(),p=new URLSearchParams({rows:'10',cpage:String(page)});if(f.genre)p.set('shcate',f.genre);if(f.area)p.set('signgucodesub',f.area);if(f.keyword)p.set('shprfnm',f.keyword);fetch('/api/performances?'+p).then(function(r){if(!r.ok)throw 0;return r.text()}).then(function(xml){var items=typeof parse==='function'?parse(xml):[];if(typeof lastItems!=='undefined')lastItems=items;if(typeof renderItems==='function')renderItems(items);var doc=new DOMParser().parseFromString(xml,'text/xml'),total=Number(doc.querySelector('totalcount')?.textContent||0),pages=total?Math.ceil(total/10):(items.length===10?page+1:page);renderPagination(page,pages,total);if(count)count.textContent='공연 '+(total||items.length).toLocaleString()+'개';}).catch(function(){if(count)count.textContent='';grid.innerHTML='<div class="empty">공연 정보를 불러오지 못했습니다.</div>';if(el)el.innerHTML='';});}
function renderPagination(page,pages,total){var el=document.querySelector('#pagination');if(!el)return;if(pages<=1){el.innerHTML='';return;}var start=Math.floor((page-1)/10)*10+1,end=Math.min(pages,start+9),out=[];out.push('<button class="nav" '+(page<=1?'disabled':'')+' data-page="'+(page-1)+'">‹ 이전</button>');for(var i=start;i<=end;i++)out.push('<button class="'+(i===page?'active':'')+'" data-page="'+i+'">'+i+'</button>');out.push('<button class="nav" '+(page>=pages?'disabled':'')+' data-page="'+(page+1)+'">다음 ›</button>');el.innerHTML=out.join('');el.querySelectorAll('[data-page]').forEach(function(b){b.onclick=function(){if(!b.disabled)loadPage(Number(b.dataset.page));};});}
