...
<div class="notice-overlay" id="synopsisNotice"><div class="notice-box"><h3>공연정보 안내</h3><p>이 공연의 정보는 <strong>KOPIS 공연예술통합전산망</strong>에서 제공받습니다.<br><br>KOPIS에 등록된 줄거리 정보가 없어 MOVOKA에도 줄거리를 표시하지 않습니다.</p><label class="notice-check"><input type="checkbox" id="noticeToday"> 오늘은 더 표기하지 않기</label><button class="notice-close" type="button" onclick="closeSynopsisNotice()">확인</button></div></div>
...
/* 줄거리 안내 모달을 오늘 하루만 숨길지 확인합니다. */
function closeSynopsisNotice(){const overlay=document.getElementById('synopsisNotice');if(!overlay)return;if(document.getElementById('noticeToday')?.checked)localStorage.setItem('movoka-synopsis-notice-date',new Date().toISOString().slice(0,10));overlay.style.display='none';}
(function(){const overlay=document.getElementById('synopsisNotice');if(!overlay)return;const today=new Date().toISOString().slice(0,10);if(localStorage.getItem('movoka-synopsis-notice-date')!==today)overlay.style.display='flex';})();
