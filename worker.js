...
const BATCH_PAGES = 6; // 한 번의 Worker 호출에서 가져올 KOPIS 페이지 수입니다.
...
const DETAIL_CONCURRENCY = 5; // 상세 공연 정보를 동시에 조회할 수 있습니다.
...
async function kopis(api) { const response = await fetch(api.toString(), {redirect: 'follow'}); const text = await response.text(); if (!response.ok) throw new Error(`KOPIS HTTP ${response.status}: ${text.slice(0, 180)}`); return text; } // KOPIS를 호출하고 실제 오류 내용을 보존합니다.
...
function baseApi(url, key) { const now = new Date(); const end = new Date(now); end.setDate(end.getDate() + 30); const api = new URL('https://www.kopis.or.kr/openApi/restful/pblprfr'); api.searchParams.set('service', key); api.searchParams.set('stdate', url.searchParams.get('stdate') || ymd(now)); api.searchParams.set('eddate', url.searchParams.get('eddate') || ymd(end)); const genre = url.searchParams.get('shcate') || ''; const area = url.searchParams.get('signgucode') || url.searchParams.get('signgucodesub') || url.searchParams.get('shigucodesub') || ''; const keyword = url.searchParams.get('shprfnm') || ''; if (genre) api.searchParams.set('shcate', genre); if (area) api.searchParams.set('signgucode', area); if (keyword) api.searchParams.set('shprfnm', keyword); return api; } // 프론트 조건을 공식 KOPIS API URL로 변환합니다.
...
async function batchPages(api, start) { const first = Number(start) || 1; const pages = []; for (let page = first; page < first + BATCH_PAGES; page++) { const request = new URL(api.toString()); request.searchParams.set('cpage', String(page)); request.searchParams.set('rows', String(ROWS)); pages.push(request); } const results = await Promise.all(pages.map(kopis)); const items = []; const seen = new Set(); let done = false; for (const xml of results) { const pageItems = dbs(xml); for (const db of pageItems) { const id = idOf(db); if (id && !seen.has(id)) { seen.add(id); items.push(db); } } if (pageItems.length < ROWS) { done = true; break; } } return {items, nextStart: first + BATCH_PAGES, done}; } // 여러 페이지를 작은 묶음으로 수집합니다.
...
async function ticketStatus(ids, key) { const unique = [...new Set(ids)].filter(id => /^PF\d+$/.test(id)).slice(0, TICKET_BATCH_SIZE); const result = {}; let cursor = 0; const worker = async () => { while (cursor < unique.length) { const id = unique[cursor++]; try { const api = new URL(`https://www.kopis.or.kr/openApi/restful/pblprfr/${encodeURIComponent(id)}`); api.searchParams.set('service', key); const url = api.toString(); const xml = (await cachedText(url)) || await kopis(api); await saveText(url, xml, DETAIL_CACHE_TTL); const urls = xml.match(/<relateurl>[\s\S]*?<\/relateurl>/g) || []; result[id] = urls.some(v => /^https?:\/\//i.test(v.replace(/<\/?relateurl>/g, '').trim())); } catch (_) { result[id] = false; } } }; await Promise.all(Array.from({length: Math.min(DETAIL_CONCURRENCY, unique.length)}, worker)); return result; } // 공연 상세정보에서 등록된 예매처 URL 존재 여부를 확인합니다.
...
if (url.pathname === '/api/performance') { const id = url.searchParams.get('mt20id'); if (!id || !/^PF\d+$/.test(id)) return Response.json({error: 'Invalid mt20id'}, {status: 400}); try { const api = new URL(`https://www.kopis.or.kr/openApi/restful/pblprfr/${encodeURIComponent(id)}`); api.searchParams.set('service', key); const apiUrl = api.toString(); const xml = (await cachedText(apiUrl)) || await kopis(api); await saveText(apiUrl, xml, DETAIL_CACHE_TTL); return new Response(xml, {headers: {'Content-Type': 'application/xml; charset=utf-8', 'Cache-Control': `public, max-age=${DETAIL_CACHE_TTL}`}}); } catch (error) { return Response.json({error: String(error?.message || error).slice(0, 300)}, {status: 502}); } }
...
async function loadWithTicketFilter(){
  ...
  const firstKey='movoka-first-list-v10:'+p.toString();
  ...
  try{
    ...
    if(!response.ok){ const errorText=await response.text(); throw new Error(errorText||'first page failed'); }
    const xml=await response.text(); if(token!==movokaLoadToken)return; lastItems=parse(xml); if(!lastItems.length)throw new Error('KOPIS 응답에서 공연을 찾지 못했습니다.'); renderItems(lastItems);
    try{localStorage.setItem(firstKey,JSON.stringify({data:xml,savedAt:Date.now()}));}catch(_){ }
  }catch(error){ if(token===movokaLoadToken){ grid.innerHTML='<div class="empty">공연 정보를 불러오지 못했습니다.<br><small style="display:block;margin-top:10px">'+esc(String(error?.message||error).slice(0,220))+'</small><button class="primary" style="margin-top:14px;padding:10px 16px" onclick="loadWithTicketFilter()">다시 시도</button></div>'; } }
  $('#go').disabled=false;
  ...
  while(token===movokaLoadToken){
    try{
      ...
      if(token!==movokaLoadToken)break; mergeMovokaXml(data.xml||'<dbs></dbs>'); if(data.done)break; start=Number(data.nextStart)||start+6; await new Promise(resolve=>setTimeout(resolve,250));
    }catch(_){ break; }
  }
}
