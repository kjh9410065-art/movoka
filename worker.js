...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/' || url.pathname === '/index.html') {
      ...
      html=html.replace('</style></head>','.ad-slot{display:none;width:100%;min-height:90px;margin:0 0 24px;align-items:center;justify-content:center;overflow:hidden}.ad-slot.has-ad{display:flex}.ad-slot ins,.ad-slot iframe{max-width:100%;display:block}@media(max-width:480px){.ad-slot{min-height:60px;margin-bottom:18px}}'+ '</style></head>');
